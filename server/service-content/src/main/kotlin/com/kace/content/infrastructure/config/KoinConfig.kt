...
fun contentServiceModule(database: Database) = module {
    ...
    single<ContentCommentRepository> { ContentCommentRepositoryImpl(database) }
    single<ContentFeedbackRepository> { ContentFeedbackRepositoryImpl(database) }
    ...
    single<ContentCommentService> { ContentCommentServiceImpl(get(), get()) }
    single<ContentFeedbackService> { ContentFeedbackServiceImpl(get(), get()) }
    
    // 控制器
    single { ContentTypeController() }
    single { ContentController() }
    single { CategoryController() }
    single { TagController() }
    single { ContentRelationController() }
    single { ContentPermissionController() }
    single { ContentSearchController() }
    single { ContentCommentController() }
    single { ContentFeedbackController() }
} 