...
import com.kace.user.api.request.CreateRoleRequest
import com.kace.user.api.request.RoleCreateRequest
import com.kace.user.api.request.RoleUpdateRequest
import com.kace.user.api.request.UpdateRoleRequest
...
private fun Role.toResponse(): RoleResponse {
    return RoleResponse(
        ...
        isSystem = isSystem,
        permissions = null, // 权限列表通过单独的API获取
        createdAt = createdAt,
        updatedAt = updatedAt
    )
}
 ...
 * 验证创建角色请求
 */
private fun CreateRoleRequest.validate() {
    if (name.isBlank()) throw BadRequestException("角色名称不能为空")
}

/**
 * 验证更新角色请求
 */
private fun UpdateRoleRequest.validate() {
    name?.let { if (it.isBlank()) throw BadRequestException("角色名称不能为空") }
} 